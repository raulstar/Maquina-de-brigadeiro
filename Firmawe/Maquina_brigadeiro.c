...
#DEVICE ADC = 10 // Configura a precisão do conversor analógico-digital (ADC) para 10 bits.
...
void main()
{
  ...
  printf(lcd_putc, "e0\r\n");                               // Imprime uma mensagem no display LCD
   delay_ms(50);                                                         //  delay_ms(50);                                             // Pequeno atraso de 50ms
  printf(lcd_putc, "\fCorta Brigadeiros\n           v2.0"); // Imprime uma mensagem no display LCD
  delay_ms(900);//  delay_ms(500);
   contador = lerMemoria();
  ...
  while (true)
  {
    ...
    if (menu)
    { // Verifica se o botão de menu está pressionado
      contador = 0;
    ...
    comtroleMotor(duty, setpoit); // Controla o motor com base no duty cycle e no setpoint
     delay_ms(10);                              //   delay_ms(10);                 // Pequeno atraso de 10ms
...
int leitura(void)
{
  ...
  value = read_adc();
  value = value * 10;  // Lê o valor do ADC
  value = value / 102; // Converte o valor para um inteiro
...
void comtroleMotor(int duty, setpoit)
{
  static int velocidadeManual = 0, limiteVelocidade = 120; // Variáveis estáticas para controle da velocidade manual
...
void contagem(void)
{
  ...
  if ((menu) || (contador > 30000)) // Verifica se o botão de menu está pressionado
